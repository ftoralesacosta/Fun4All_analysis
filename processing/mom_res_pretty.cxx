...
int main(int argc, char ** argv) {
	...
	for(int i = 8 ; i < num_eta_bin[0] ; i++){
		...
		leg[i] -> AddEntry(h1_dpp_v_p_et_bins[0][i]," ");//Form("3.0T, A = %.3f, B = %.3f",f_dpp_10um_30T[i]->GetParameter(0),f_dpp_10um_30T[i]->GetParameter(1)));
		...
		// tex_1_4_T_par[i] = new TLatex(3.5,tex_y_1_4[i-8],Form("#bf{1.4T, A = %.3f, B = %.3f}",f_dpp_10um_14T[i]->GetParameter(0),f_dpp_10um_14T[i]->
		//								      GetParameter(1)));
	...
	//for(int i = 8 ; i < num_eta_bin[0] ; i++){
		//c1 -> cd(i-7);
	        pad1[i] = c1 -> cd(i-7);
                pad1[i] -> SetPad(Form("pad1_%i",i-8),Form("pad1_%i",i-8),lowx[i-8],lowy[i-8],higx[i-8],higy[i-8],kWhite, 0, 0);
