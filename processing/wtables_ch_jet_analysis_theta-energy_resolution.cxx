...
int main(int argc, char ** argv) {
	...
	for(int et = 0 ; et < size_eta_bin-1 ; et++){
	  if (N_Missing_Cut)
	    h1_dpp_v_p_et_bins[et] = new TH1F(Form("h1_dpp_v_p_et_bins_%i",et),";P [GeV/c];dP/P [%]",size_mom_bin-1,mom_bin);
	  else
	    h1_dpp_v_p_et_bins[et] = new TH1F(Form("h1_dpp_v_p_et_bins_%i",et),";P [GeV/c];StdDev dP/P [%]",size_mom_bin-1,mom_bin);
	...
	for(int p = 0 ; p < size_mom_bin-1 ; p++){
	  if (N_Missing_Cut)
	    h1_dpp_v_et_p_bins[p] = new TH1F(Form("h1_dpp_v_et_p_bins_%i",p),";#eta;dP/P [%]"      ,size_eta_bin-1,eta_bin);
	  else
	    h1_dpp_v_et_p_bins[p] = new TH1F(Form("h1_dpp_v_et_p_bins_%i",p),";P [GeV/c];StdDev dP/P [%]",size_eta_bin-1,mom_bin);
...
//					approx_mean_dpp[et][p]-approx_sig_dpp_3_0[et][p],approx_mean_dpp[et][p]+approx_sig_dpp_3_0[et][p]);
 ...
 // if (eta_bin[et] < 1){
	...
	// TGraphErrors **	tge_dpp_v_p_et_bins = Th1_to_TGraph(h1_dpp_v_p_et_bins,size_eta_bin-1);
        // TGraphErrors ** tge_dth_v_p_et_bins = Th1_to_TGraph(h1_dth_v_p_et_bins,size_eta_bin-1);
        // TGraphErrors ** tge_dph_v_p_et_bins = Th1_to_TGraph(h1_dph_v_p_et_bins,size_eta_bin-1);
        // TGraphErrors ** tge_dpp_v_et_p_bins = Th1_to_TGraph(h1_dpp_v_et_p_bins,size_mom_bin-1);
        // TGraphErrors ** tge_dth_v_et_p_bins = Th1_to_TGraph(h1_dth_v_et_p_bins,size_mom_bin-1);
        // TGraphErrors ** tge_dph_v_et_p_bins = Th1_to_TGraph(h1_dph_v_et_p_bins,size_mom_bin-1);


	//cout<<__LINE__<<": "<<tge_dpp_v_p_et_bins[1]->GetTitle()<<" bin content = "<<tge_dpp_v_p_et_bins[1]->GetPointY(2)<<endl;
